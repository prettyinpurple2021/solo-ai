...
- **Billing API unification:** Centralized subscription payload, Stripe checkout, billing portal, cancel, and reactivate flows in **`src/lib/billing/`** (`subscription-payload`, `checkout`, `portal`, `subscription-lifecycle`). **`/api/billing/subscription`** now returns a unified shape (`tier`, `status`, `current_period_end`, nested `subscription`) sourced from real **`users`** columns (fixes incorrect **`subscription_current_period_end`** reference). **`/api/billing/checkout`** supports optional **`billing`** (`monthly` \| `yearly`), reuses **`SUBSCRIPTION_TIERS`** / env price IDs, ensures a Stripe customer via DB lookup (works with JWT sessions that omit `stripe_customer_id`). Added **`/api/billing/cancel-subscription`** and **`/api/billing/reactivate-subscription`**. **`/api/stripe/*`** counterparts delegate to the same helpers so legacy callers stay compatible. UI entry points (**`dashboard/billing`**, **`subscription-manager`**, **`subscription-guard`**, **`pricing-cards`**) now call **`/api/billing/*`**; **`api-client`** maps **`endpoints.stripe.*`** to billing paths for consistency.
- **Billing subscription auth isolation:** Hardened **`src/app/api/billing/subscription/route.ts`** to enforce session identity as source of truth. Query `userId` is now rejected with **403** when it does not match the authenticated user, preventing cross-account subscription reads.
- **Temporal API compatibility layer:** Added **`/api/temporal/onboarding`**, **`/api/temporal/intelligence`**, and **`/api/temporal/briefings`** routes with authenticated execution + status retrieval, plus **`src/lib/temporal-workflow-store.ts`** for workflow status persistence (Upstash Redis with in-memory fallback). This resolves frontend 404s for temporal workflow hooks and returns consistent workflow payloads.
- **Competitor-specific alerts contract hardening:** Updated **`src/hooks/use-competitor-alerts.ts`** to normalize both supported response formats for `/api/competitors/[id]/alerts` (array or `{ alerts: [] }`) to prevent empty-state regressions.
- **Validation:** Ran **`npm run validate`** after changes; lint + web/server type-check passed.
