...
interface IntelligenceClientProps {
  initialData: IntelligencePageData;
  userId: string;
}

export default function IntelligenceClient({ initialData, userId }: IntelligenceClientProps) {
  ...
  const { insights, stats, market_position: marketPosition, strategic_analysis: strategicAnalysis } = initialData
  ...
  const filteredInsights = insights.filter(insight => {
    ...
    const matchesImportance = filterImportance === 'all' || insight.importance.toLowerCase() === filterImportance.toLowerCase()
    const matchesType = filterType === 'all' || insight.type.toLowerCase() === filterType.toLowerCase()
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge className="bg-red-500/20 text-red-500 border-red-500/30">
                          {insights.filter(i => i.importance.toLowerCase() === 'critical').length}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-purple-200">High Priority</span>
                        <Badge className="bg-orange-500/20 text-orange-500 border-orange-500/30">
                          {insights.filter(i => i.importance.toLowerCase() === 'high').length}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-purple-200">Medium Priority</span>
                        <Badge className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30">
                          {insights.filter(i => i.importance.toLowerCase() === 'medium').length}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-purple-200">Low Priority</span>
                        <Badge className="bg-green-500/20 text-green-500 border-green-500/30">
                          {insights.filter(i => i.importance.toLowerCase() === 'low').length}
                        </Badge>
                      </div>
                    </div>
                  </CardContent>
