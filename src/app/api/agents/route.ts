...
const AVAILABLE_AGENTS = [
  {
    id: 'roxy',
    name: 'Roxy',
    description: 'Business Coaching & Growth Agent',
    capabilities: ['Strategy', 'Motivation', 'Accountability']
  },
  {
    id: 'lexi',
    name: 'Lexi',
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (payload.stream) {
      ...
      logApi('POST', '/api/agents', 200, Date.now() - start, { streamed: true })
      return new Response(stream, {
        headers: SSE_HEADERS,
      })
    }

    logApi('POST', '/api/agents', 202, Date.now() - start, { jobId: job.id })
    return NextResponse.json(
      {
        success: true,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // If requesting available agents list
    if (searchParams.get('list') === 'available' || !searchParams.has('jobId')) {
       return NextResponse.json({ success: true, data: AVAILABLE_AGENTS });
    }
    ...
    if (jobId) {
      ...
      logApi('GET', '/api/agents', 200, Date.now() - start, { jobId })
    ...
    logApi('GET', '/api/agents', 200, Date.now() - start, { jobs: jobs.length })
    ...
    logError('Error fetching agent data', error)
    return NextResponse.json(
