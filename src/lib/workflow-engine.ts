    ...
    this.registerNodeType({
      ...
      execute: async (config: unknown, _context) => {
        ...
        try {
          ...
          const { isEmailConfigured, sendTransactionalEmail, getDefaultFromAddress } =
            await import('@/lib/mail-transport')

          if (!isEmailConfigured()) {
            throw new Error('SMTP email is not configured. Email cannot be sent.')
          }
          ...
          if (!result.success) {
            throw new Error(`Email Error: ${result.error ?? 'send failed'}`)
          }

          return { sent: true, messageId: result.messageId, timestamp: new Date().toISOString() }
        } catch (error) {
    ...
    this.registerNodeType({
      ...
      execute: async (config: unknown, context) => {
        ...
        try {
          ...
          const { AgentCollaborationSystem } = (await import('@/lib/custom-ai-agents/agent-collaboration-system')) as unknown as {
            AgentCollaborationSystem: new (userId: string) => {
              processRequest: (
                request: string,
                context?: Record<string, unknown>,
                primaryAgentId?: string
              ) => Promise<{
                primaryResponse: { content: string; confidence: number }
              }>
            }
          }
