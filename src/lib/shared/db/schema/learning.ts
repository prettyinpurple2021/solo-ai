...
export const userSkills = pgTable('user_skills', {
    ...
    chkLevel: check('chk_user_skills_level', sql`${table.current_level} >= 1`),
    chkXp: check('chk_user_skills_xp', sql`${table.current_xp} >= 0`),
}));
...
export const assessments = pgTable('assessments', {
    ...
    chkPassingScoreRange: check('chk_learning_passing_score_range', sql`${table.passing_score} BETWEEN 0 AND 100`),
}));
