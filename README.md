...
4. **Live site:** **Vercel** runs the **Next.js** frontend (and its `/api/*` routes). **Railway** runs the **Express** backend (`server/`, WebSockets). Pushes to **`main`** can deploy both via Gitea Actions (see [`.gitea/workflows/deploy.yaml`](.gitea/workflows/deploy.yaml)); optional Railway deploy is enabled only if you add the secrets listed in [`.gitea/README.md`](.gitea/README.md). **Which env var goes where:** **[docs/deployment/ENV_VARS_VERCEL_AND_RAILWAY.md](docs/deployment/ENV_VARS_VERCEL_AND_RAILWAY.md)**. Short version: same **`DATABASE_URL`** and **`JWT_SECRET`** on both; **`NEXT_PUBLIC_SOCKET_URL`** only on **Vercel**, set to your Railway **https** root URL.
